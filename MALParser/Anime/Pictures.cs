...
namespace MALParser.Anime
{
    public class Pictures
    {
        ...
        public static async Task<PicturesPage> ParseAsync(string link)
        {
            return AnalyzeDocument(await client.GetStringAsync(link));
        }

        public static PicturesPage Parse(string link)
        {
            return AnalyzeDocument(client.GetStringAsync(link).Result);
        }

        private static PicturesPage AnalyzeDocument(string HTMLCode)
        {
            HtmlDocument doc = new HtmlDocument();
            doc.LoadHtml(HTMLCode);

            PicturesPage page = new PicturesPage(Header.Parse(HTMLCode));
